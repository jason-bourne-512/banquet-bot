...
Labels: a named menu package (e.g. "Tejas Mexicano," "Healthy Start") shows its real name, shortened; a plain coffee/tea break with no named package shows REG/DEC/HW/Pepsi/Coke/etc.; a plated meal with no named package shows the priciest listed dish (usually the entrée); non-F&B activities (meeting, setup, registration, general session, breakout, exhibits, storage, teardown, office, green room, waiting room, board meeting) get short activity codes. (The legend block explaining these codes was removed 2026-09-04 per Jason's call — codes show with no key now.)

Look: always renders in a black-background/green-glow "digital terminal" style regardless of the site's own light/dark toggle (a deliberate distinct mode, not theme-linked) — random words briefly scramble into glitch characters. (The horizontal scan-beam sweep was tried and then removed 2026-09-04 per Jason's call — just the word-glitch remains.) Today gets a TODAY badge, matching the Rundown tab's conventions.

**Also 2026-09-04 — hard rule, both tabs:** Rundown and Overview no longer show days before today at all (previously past days stayed visible but dimmed). Now they're actually removed from the list — see [[feedback_banquet_bot_hide_past_days]] in memory.
